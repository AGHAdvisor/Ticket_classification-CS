...
if "Brief Description of Feedback" in train_df.columns:
    train_features = train_df['Brief Description of Feedback'].fillna('').apply(clean_text)
elif "Message" in train_df.columns:
    train_features = train_df['Message'].fillna('').apply(clean_text)
else:
    st.error("Error: 'Brief Description of Feedback' or 'Message' column not found in the training dataset.")
    exit()
